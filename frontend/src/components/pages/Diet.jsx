...
export default function Diet() {
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 pb-24">
            ...
            {/* Header Card */}
            <div className="bg-white dark:bg-slate-800/80 rounded-3xl shadow-lg border border-slate-100 dark:border-slate-700 p-6 mb-6 relative overflow-hidden transition-colors">
                <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-emerald-100/50 to-teal-100/50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-full blur-3xl -mr-20 -mt-20" />

                <div className="relative flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-2xl flex items-center justify-center text-white text-3xl shadow-xl shadow-emerald-200 dark:shadow-emerald-900/30">
                            🥗
                        </div>
                        <div>
                            <h1 className="text-2xl font-bold text-slate-800 dark:text-white">My Diet Plan</h1>
                            <div className="flex items-center gap-3 mt-1">
                                <span className="text-sm text-slate-500 dark:text-slate-400">{formatDate(selectedDate)}</span>
                                <span className="w-1 h-1 bg-slate-300 dark:bg-slate-600 rounded-full" />
                                <span className="text-sm font-medium text-emerald-600 dark:text-emerald-400">{totalRecipes} meals</span>
                                <span className="w-1 h-1 bg-slate-300 dark:bg-slate-600 rounded-full" />
                                <span className="text-sm font-medium text-orange-600 dark:text-orange-400">~{totalCalories} kcal</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        {/* Quick stats */}
                        <div className="hidden sm:flex items-center gap-4 px-4 py-2 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-100 dark:border-slate-600">
                            <div className="text-center">
                                <p className="text-lg font-bold text-amber-600 dark:text-amber-400">{diet.morning.length}</p>
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase">Morning</p>
                            </div>
                            <div className="w-px h-8 bg-slate-200 dark:bg-slate-600" />
                            <div className="text-center">
                                <p className="text-lg font-bold text-sky-600 dark:text-sky-400">{diet.afternoon.length}</p>
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase">Afternoon</p>
                            </div>
                            <div className="w-px h-8 bg-slate-200 dark:bg-slate-600" />
                            <div className="text-center">
                                <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400">{diet.night.length}</p>
                                <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase">Night</p>
                            </div>
                        </div>
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${showDates
                                    ? 'bg-gradient-to-r from-emerald-400 to-teal-500 text-white shadow-lg shadow-emerald-200 dark:shadow-emerald-900/30'
                            ...
                            {/* Dropdown */}
                            {showDates && (
                                <div className="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 p-2 z-50 animate-fadeIn">
                                    <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase px-3 py-2">Select Date</p>
            ...
            {/* Toast Notification */}
            {deleteMsg && (
                <div className="fixed top-24 right-6 z-50 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xl rounded-2xl px-5 py-3 flex items-center gap-2 animate-bounce transition-colors">
                    <span className="text-emerald-500">✓</span>
                    <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{deleteMsg}</span>
                </div>
            )}
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20">
                    <div className="w-12 h-12 border-3 border-emerald-200 dark:border-emerald-800 border-t-emerald-500 rounded-full animate-spin mb-4" />
                    <p className="text-slate-400 dark:text-slate-500">Loading your diet plan...</p>
                </div>
            ) : (
                /* Meal Cards Grid */
                    ...
                    {SLOTS.map(slot => {
                        ...
                        return (
                            <div
                                ...
                                className={`group bg-white dark:bg-slate-800/80 rounded-3xl shadow-sm border-2 ${slot.border} overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1 cursor-pointer ${recipes.length === 0 ? 'opacity-80' : ''}`}
                            >
                                {/* Header */}
                                ...
                                <div className={`p-5 ${slot.bg} min-h-[280px]`}>
                                    {recipes.length === 0 ? (
                                        <div className="flex flex-col items-center justify-center h-full py-8 text-center">
                                            ...
                                            <p className="text-slate-500 dark:text-slate-400 font-medium">No recipes planned</p>
                                            <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">Tap to add meals</p>
                                        </div>
                                    ) : (
                                        <div className="space-y-3">
                                            {recipes.slice(0, 3).map((recipe, i) => (
                                                <div
                                                    ...
                                                    className="bg-white dark:bg-slate-800 rounded-xl p-3 shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-all group/card"
                                                >
                                                    <div className="flex items-start justify-between">
                                                        ...
                                                        <button
                                                            ...
                                                            className="opacity-0 group-hover/card:opacity-100 transition-opacity p-1 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"
                                                        >
                                                    ...
                                                    <div className="flex gap-2 mt-2">
                                                        {recipe.calories && (
                                                            <span className="text-[10px] bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-full text-slate-600 dark:text-slate-300 font-medium">
                                                                🔥 {recipe.calories}
                                                            </span>
                                                        )}
                                                        {recipe.protein && (
            ...
            {selectedSlot && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={closeModals}>
                    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-hidden animate-fadeIn" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="space-y-4">
                                {diet[selectedSlot.key].map((recipe, i) => (
                                    <div
                                        ...
                                        className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-all cursor-pointer"
                                    >
            ...
            {selectedRecipe && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={closeModals}>
                    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden animate-fadeIn" onClick={e => e.stopPropagation()}>
                        {/* Header */}
                        ...
                        <div className="p-6 overflow-y-auto max-h-[60vh]">
                            ...
                            {selectedRecipe.ingredients && selectedRecipe.ingredients.length > 0 && (
                                <div className="mb-6">
                                    <h4 className="font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2">
                                        <span className="w-8 h-8 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400">📝</span>
                                        Ingredients
                                    </h4>
                                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        {selectedRecipe.ingredients.map((ing, i) => (
                                            <li key={i} className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-700/50 p-2 rounded-lg">
                            ...
                            {selectedRecipe.reason && (
                                <div className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-2xl p-4 border border-emerald-100 dark:border-emerald-800">
                                    <h4 className="font-bold text-emerald-800 dark:text-emerald-300 mb-2 text-sm">Why this recipe?</h4>
                                    <p className="text-sm text-emerald-700 dark:text-emerald-400 leading-relaxed">{selectedRecipe.reason}</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                            <button onClick={closeModals} className="px-6 py-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-medium hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
                                Close
                            </button>
                        </div>
                    </div>
